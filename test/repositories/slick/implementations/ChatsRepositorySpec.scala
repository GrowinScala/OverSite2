...
class ChatsRepositorySpec extends AsyncWordSpec with MustMatchers with OptionValues with Inside with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  "SlickChatsRepository#getEmail" should {
    ...
    "return a chat with one email for an allowed user" in {
      val expectedResponse = Chat(chatId, "Location",
      ...
      chatsRep.getEmail(chatId, emailId, authorizedUserId).map(chat => chat.value mustBe expectedResponse)
    }
    ...
    "return a chat with a draft if the user is its owner" in {
      ...
      chatsRep.getEmail(chatId, draftEmailId, authorizedUserId).map(chat => chat.value mustBe expectedResponse)
    }
    ...
    "return None if the email is a draft of another user" in {
      ...
      chatsRep.getEmail(chatId, draftEmailId, notDraftOwnerUserId).map(chat => chat mustBe None)
    }
    ...
    "return None for user that is not allow to see the requested email" in {
      ...
      chatsRep.getEmail(chatId, emailId, unauthorizedUserId).map(chat => chat mustBe None)
    }
